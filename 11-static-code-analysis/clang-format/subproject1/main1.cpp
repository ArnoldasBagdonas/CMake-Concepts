...
void integerOverflow() {
  ...
  std::cout << "Overflowed value: " << overflowed
            << std::endl; // Integer overflow (High severity)
}
...
void unreachableCode(int x) {
  if (x > 10) {
    ...
    std::cout << "This line is unreachable."
              << std::endl; // Unreachable code (High severity)
  }
}

// 13. Unused function parameter
void unusedParameter(int x, int /*unused*/) {
  std::cout << "Used parameter: " << x
            << std::endl; // Unused function parameter (High severity)
}
